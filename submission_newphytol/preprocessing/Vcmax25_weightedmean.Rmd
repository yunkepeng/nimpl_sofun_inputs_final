...
location <- "~/data/nimpl_sofun_inputs/map/Final_nc_file_vcmax25_mean"
...
inputnc <- function(name,start_year,end_year){
  ...
  for (i in firstyr_data:endyr_data){
    nc <- read_nc_onefile(alloutput_list[i-firstyr_data+1]) #Input nc
...
vcmax25_df_output <- vcmax25_df[,c("lon","lat","vcmax25")]
vcmax25_nc <- list(df_to_grid(vcmax25_df_output,varnam = "vcmax25", lonnam = "lon", latnam = "lat"))
names(vcmax25_nc) <- "vcmax25"
varams = "vcmax25"
test <- list(lon,lat,vcmax25_nc,varams)
names(test) <- c("lon","lat","vars","varams")
write_nc2(test,varnams = "vcmax25",long_name = "vcmax25",units = "umol/m2/s",
          path = "~/data/nimpl_sofun_inputs/map/Final_ncfile/vcmax25_weighted.nc")
